...
template<typename T> typename List<T>::ReveseIterator List<T>:: rbegin()
{
	return Tail;
}
template<typename T> typename List<T>::ReveseIterator List<T>:: rend()
{
	return nullptr;
}
...
{
	//RAII - Resource Aqusition Is Initialisation (Выделение ресурсов это инимциализация)
...
void main()
{
	...
	for (List<double>::ReveseIterator rit = d_list.rbegin(); rit != d_list.rend(); ++rit)
	{
		cout << *rit << tab;
	}
	cout << endl;
	...
	for (List<std::string>::ReveseIterator rit = s_list.rbegin(); rit != s_list.rend(); ++rit)
	{
		cout << *rit << tab;
	}
	cout << endl;
	...
	/*
	Class object;		//обьявление объекта обычного класса
	Class<type> object; //обьявление объекта шаблонного класса
